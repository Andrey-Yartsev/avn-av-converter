...
class LocalDriver extends Driver
{
    ...
    protected function resizeImage($filePath, $width, $height, $blur = null)
    {
        $image = $this->imagine->open($filePath);
        if ($width && empty($height)) {
            $imageSize = $image->getSize();
            $height = $imageSize->getHeight() / ($imageSize->getWidth() / $width);
        } elseif ($height && empty($width)) {
            $imageSize = $image->getSize();
            $width = $imageSize->getWidth() / ($imageSize->getHeight() / $height);
        ...
        $this->result[] = new ImageResponse([
            'size' => filesize(PUBPATH . $savedPath),
            'width' => $width,
            'height' => $height,
            'url' => $url
        ]);
